...
public class RdfExporterTest {
  ...
  @Mock private MolgenisClient molgenisClient;
  @Mock private RdfTemplate rdfTemplate;
  @Mock private RepositoryConnection repositoryConnection;
  @Mock private EntityConsumerFactory consumerFactory;
  @Mock private EntityConsumer entityConsumer;
  @Mock private Entity samples;
  @Mock private Entity biobanks;
  @Mock private MolgenisVersion molgenisVersion;
  @Mock private MetadataRepository metadataRepository;
  @Mock private RdfConfig rdfConfig;
  @Captor private ArgumentCaptor<ConnectionCallback> connectionCallbackCaptor;
  ...
  public void testExportData() throws Exception {
    doReturn(ImmutableMap.of(RDF.PREFIX, RDF.NAMESPACE, FOAF.PREFIX, NAMESPACE))
        .when(rdfConfig)
        .getNamespaces();
    ...
    doReturn(entityConsumer)
        .when(consumerFactory)
        .create(biobanks, repositoryConnection, rdfConfig);
    doReturn(entityConsumer).when(consumerFactory).create(samples, repositoryConnection, rdfConfig);

    rdfExporter.exportData(
        ImmutableList.of("bbmri_nl_sample_collections", "bbmri_nl_biobanks"),
        pageSize,
        molgenisVersion);
    connectionCallbackCaptor
        .getAllValues()
        .forEach(value -> value.doInConnection(repositoryConnection));

    verify(molgenisClient)
        .streamEntityData("bbmri_nl_sample_collections", entityConsumer, pageSize);
  ...
  public void testExportEntity() throws Exception {
    ...
    when(consumerFactory.create(samples, repositoryConnection, rdfConfig))
        .thenReturn(entityConsumer);
