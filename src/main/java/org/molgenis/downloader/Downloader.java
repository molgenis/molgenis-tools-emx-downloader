...
public class Downloader {
  ...
  private static OptionParser createOptionParser() {
    ...
    parser
        .acceptsAll(asList("f", FILE), "Name of the file to write the data to.")
        .withRequiredArg()
        .ofType(File.class)
        .required();
    parser.acceptsAll(asList("o", OVERWRITE), "Overwrite the file if it exists.");
    parser
        .acceptsAll(asList("u", URL), "URL of the MOLGENIS instance")
        .withRequiredArg()
        .ofType(String.class)
        .required();
    parser.acceptsAll(
        asList("D", DATA_ONLY), "Write only the data for the entities to the output file.");
    parser
        .acceptsAll(asList("a", ACCOUNT), "MOLGENIS username to login with to download the data.")
        .withRequiredArg()
        .ofType(String.class);
    parser
        .acceptsAll(asList("p", PASSWORD), "Password for the MOLGENIS user to login")
        .withRequiredArg()
        .ofType(String.class);
    parser.acceptsAll(
        asList("i", INSECURE_SSL), "Ignore SSL certicate chain errors and hostname mismatches.");
    parser
        .acceptsAll(
            asList("s", PAGESIZE),
    ...
    parser
        .acceptsAll(asList("v", VERSION), "Overrides the result from '/api/v2/version'")
        .withRequiredArg()
        .ofType(String.class);
    parser
        .acceptsAll(
            asList("t", SOCKET_TIMEOUT), "The socket timeout in seconds, default value is 60")
        .withRequiredArg()
        .ofType(Integer.class);
    parser
        .accepts(
            ...
            "The default namespace for newly created IRIs in RDF download, and the prefix to use. "
                ...
                + "Default value is 'mlg:http://molgenis.org/'.")
        .withRequiredArg()
        .ofType(String.class);
    parser
        .accepts(
  ...
  private void run(OptionSet options) throws Exception {
    ...
    Integer socketTimeout =
        options.hasArgument(SOCKET_TIMEOUT)
            ? (Integer) options.valueOf(SOCKET_TIMEOUT)
            : DEFAULT_SOCKET_TIMEOUT;

    if (options.has(DEBUG)) {
      ch.qos.logback.classic.Logger root =
          (ch.qos.logback.classic.Logger) LoggerFactory.getLogger("org.molgenis");
      ...
      if (options.has(RDF)) {
        ...
        if (options.has(DEFAULT_NAMESPACE)) {
          ...
          if (parts.length != 2 || !parts[1].contains(":")) {
            LOG.error(
                "Error parsing default namespace option '{}'.\n"
                    + "Make sure that you provide a prefix, followed by a :, followed by a valid URI.\n"
                    + "For example: '--defaultNamespace ex:http://example.com/'",
