...
class RdfEntityConsumer implements EntityConsumer
{
	 ...
	 */
	public static EntityConsumer create(Entity entity, RepositoryConnection connection, RdfConfig rdfConfig)
	{
	...
	{
		return valueFactory.createIRI(rdfConfig.getDefaultNamespace(), format("%s/%s", entityTypeId, entityId));
	}
	...
	private void createStatements(IRI subject, String attributeName, String attributeValue)
	{
		...
		attributeTags.stream()
					 .filter(tag -> IS_ASSOCIATED_WITH_URI.equals(tag.getRelationIRI()))
					 .forEach(tag -> tryCreateAttributeValueStatement(subject, attribute, tag.getObjectIRI(),
							 attributeValue));
	}

	private void tryCreateAttributeValueStatement(IRI subject, Attribute attribute, String tagObjectIRI,
	...
	private void createAttributeValueStatement(IRI subject, Attribute attribute, String tagObjectIRI,
			String attributeValue)
	{
