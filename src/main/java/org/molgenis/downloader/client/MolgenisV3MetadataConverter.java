...
class MolgenisV3MetadataConverter extends AbstractMetadataConverter {
  ...
  public Entity toEntity(Map<String, String> data) {
    ...
    repository
        ...
        .forEach(
            lang ->
                setString(data, "label-" + lang.getCode(), label -> entity.addLabel(label, lang)));
    return entity;
  }
  ...
  public void postProcess(WriteableMetadataRepository repository) {
    ...
    repository.getEntities().stream()
        .filter(entity -> entity.getId() != null)
        .forEach(entity -> entityIdMap.put(entity.getId(), entity));
    repository.getPackages().stream()
    ...
    repository.getEntities().stream()
        ...
        .forEach(
            entity ->
                entity.setFullName(
                    NameUtils.getPackageFullName(entity.getPackage(), VERSION_3)
                        + "_"
                        + entity.getFullName()));
    repository.getAttributes().stream()
